...
2. 온보딩에서 음성 인식으로 반려동물 이름을 받고, 사진 업로드를 선택할 수 있다.
3. 온보딩 종료 후 `/login` 또는 `/signup`으로 이동한다.
4. 로그인은 NextAuth `Credentials` provider를 통해 Spring `/api/auth/login`에 위임된다.
5. 로그인 후 `/home`, `/finance`, `/health`는 Next proxy에서 토큰이 없는 경우 차단된다.
6. 이후 건강, 가계부, 상품, 대화 기능이 각각 Spring API와 연결된다.
...
- `/onboarding/[step]`
  ...
  - 상태는 query string으로 다음 단계에 넘긴다.
  - `sessionStorage`에 “온보딩 내부 진입”과 “TTS unlocked” 상태를 저장한다.

특징:

- 온보딩은 실제 계정/DB에 반려동물을 등록하지 않는다.
- 이름과 이미지가 온보딩 화면 연출용으로만 이어진다.
...
- `pets` 배열이 비어 있고 `spendingData`도 하드코딩이다.
...
- 리포트, 그래프, 상품 연결, 뱃지 모달 모두 목업 데이터다.
...
- 특히 `FinanceController`, `PetService`, 일부 의료 기능은 의도치 않은 기본 사용자 fallback 경로가 있다.
...
### 9.3 `/api/pets` 목록 조회가 전체 반려동물을 반환함

- `PetService.getPets()`는 `petRepository.findAll()`을 호출한다.
- 사용자 스코프가 전혀 없다.
...
현재 MGK는 “반려동물 관리 + 금융 연결”이라는 큰 방향은 분명하고, 특히 의료 OCR, 예방접종, 산책 기록, 인증 플로우는 실제 코드가 꽤 이어져 있다. 반면 홈/설정/금융 리포트는 아직 제품용 정합성보다 데모용 UI 비중이 높다.
...
4. 저장소 내 평문 비밀값 분리
5. 홈/설정/금융 메인 목업을 실제 API와 연결
6. 죽어 있는 상품 추천 API를 노출하고 프론트 리포트와 연결
